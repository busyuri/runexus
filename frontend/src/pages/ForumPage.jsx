...
export default function ForumPage() {
    ...
    useEffect(() => {
        api.get("/entries")
            .then(async (res) => {
                const enrichedEntries = await Promise.all(res.data.map(async (entry) => {
                    try {
                        const userRes = await api.get(`/users/${entry.userId}`);
                        return {
                            ...entry,
                            userName: userRes.data.name || "Unknown",
                            userProfilePicture: userRes.data.profilePicture || null
                        };
                    } catch (err) {
    ...
    const handleSaveEntryEdit = async () => {
        try {
            ...
            await api.put(`/entries/${editingEntry}`, {
                userId: currentUserId,
            ...
            const updated = entries.map((e) =>
                e.id === editingEntry ? { ...e, description: editingText, edited: true } : e
            );
            setEntries(updated);
            ...
            console.error("Entry güncellenemedi:", err);
        }
    };
        ...
        <div className="bg-gray-100 min-h-screen py-10 px-4">
            ...
            {selectedEntry && (
              ...
              />
                                    ...
                                    {/* Yorum sahibine özel üç nokta menüsü */}
